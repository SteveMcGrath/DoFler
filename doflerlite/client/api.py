...
def login():
    ...
    # we need.
    response = opener.open('http://localhost:%s/login' % config.get('WebServer', 'port'), 
                           urlencode(data))
...
def account(username, password, info, proto, parser):
    ...
    opener.open('http://localhost:%s/post/account' % config.get('WebServer', 'port'),
                urlencode(data))
...
def image(filename):
    ...
    '''
    try:
        opener.open('http://localhost:%s/post/image' % config.get('WebServer', 'port'),
...
def stat(proto, count):
    ...
    # And submitting the data to the dofler server.
    opener.open('http://localhost:%s/post/stats' % config.get('WebServer', 'port'),
                urlencode(data))
...
def reset(ctype):
    ...
    '''
    opener.open('http://localhost:%s/post/reset/%s' % (
